...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="../.github/assets/agent-schema-dark.svg">
  <img alt="A cron or a GitHub event starts a turn in the app runtime. The agent reads the repo and vendor pages in a Vercel Sandbox whose git push is refused at the firewall. Writes go through the github tools and the trust gate to agent branches, pull requests and issues on GitHub, which Benjamin merges." src="../.github/assets/agent-schema.svg">
</picture>
...
agent/
  ...
  hooks/sandbox-refresh.ts          per-turn network policy refresh, checkout put on the branch the session last pushed to
  ...
  skills/pr-review/SKILL.md         a person's data pull request, read against the vendor pages it cites
  ...
  tools/agent.ts                    disables eve's built-in copy-of-the-root tool, whose fresh state forgets the thread and the branches
  subagents/reviewer/               a diff read the way the site renders it, on Opus, before any pull request opens or grows; its own tools/ turns eve's question tool off
...
## What the pull request review does

A pull request from a person is a claim about vendor pages, and CI only checks that the claim is well formed. So a data pull request starts a review turn that reads those pages today: every figure the diff adds or changes is compared with the page, every capture the diff carries is diffed against a fresh one from `page-text.mjs`, `pnpm validate` runs on the branch merged with main, and the reviewer subagent reads the result the way the site renders it. What comes back is one comment with the findings, file and field each, what the diff says and what the page says, or "No findings" and the list of pages read. The turn writes nothing: its principal is in no allow-list, and a review that could push is a review that could edit the branch it judges. Benjamin merges.
...
- For something narrower and read-only, `npx eve invoke "Load the pricing-watch skill and check only cursor"`. That turn's principal is in none of the trust allow-lists, so it reads and reports, and every `github__` write refuses.
...
- A person opens, reopens, pushes to or un-drafts a pull request that touches `content/` or `public/logos/`: an unattended review turn under a principal that is in no allow-list at all. It brings the branch in, runs `pnpm validate`, re-reads the pages the diff cites, captures them itself next to the contributor's captures, hands the diff to the reviewer subagent, and its last message is the comment. It cannot push, comment elsewhere, open or close anything. Three comments per pull request and a push starts nothing more, so a contributor cannot buy a sandbox and a set of vendor reads per commit; a mention on the pull request still works, under the visitor rules. Bots' pull requests are skipped, the agent's own included, which already went through the reviewer before they opened. The GitHub App has to be subscribed to `pull_request` events for any of this to fire.
...
- One prompt eve raises on its own: when a session crosses its token cap on a channel turn, it parks and asks for a fresh budget instead of failing. The default `input.requested` handler posted that question into #67 as a bot comment, token figure, numbered Approve and Stop, "answer by mentioning me". The channel renders it itself now. On Benjamin's own turn it says how to answer, because anything else he writes there queues behind the prompt and eve does not ask twice, and a comment that is nothing but one of those answers is dropped before dispatch unless he wrote it: the channel strips the mention and matches the rest against the options, and a reply that reaches a parked session answers its prompt whoever wrote it. On anyone else's turn it posts the one line every unfinished turn posts, logs the figures, and re-keys the parked session to an address no webhook resolves, since a parked session would otherwise queue every later comment and label on that thread behind an answer nobody may give. The thread's own address comes free and the next mention or label starts a fresh session. That one line carries an HTML comment as a marker, and `agentComments` reads it back: a thread holding nothing but it is still unanswered, so relabelling starts the responder again, while the visitor cap still counts the turn that posted it. `session.failed` only logs, since it fires right after `turn.failed` on the same failure.
...
The browser cannot open `github.com`, and that is a consequence of the brokering rather than a bug in either. The firewall terminates TLS only on domains carrying a `transform` rule, which is `github.com` alone, and it presents a per-sandbox CA that lands in the system trust store. The browser keeps its own store, so it reads that certificate as `ERR_CERT_AUTHORITY_INVALID`. Vendor pages are matched on SNI and forwarded without termination, so the sweep is unaffected; GitHub is read through the `github__*` tools or out of the checkout, never `web_fetch`, which carries no credential and is rate limited to almost nothing. Fixing it properly means importing `/usr/local/share/ca-certificates/vercel-proxy-ca.pem` into the browser's own trust store per session, which is more moving parts than a domain the browser has no reason to visit is worth.
