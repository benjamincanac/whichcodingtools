...
3. Decide against the page, never against the report:
   - **The page agrees with the reporter**: fix the field, bump `verified_at` on the source line you re-read and no other, write the snapshot when the field is a price, run `pnpm validate`, then push to `agent/<slug>-outdated-<YYYY-MM-DD>` with message `data(<slug>): <what changed>` and open a pull request that links this issue and shows the before and after with the URL and the date.
