...
2. Re-read the vendor page that covers that field. A page `page-text.mjs` exits 3 on is one the vendor's robots.txt reserves: say so in the reply and stop, without the browser and without another surface. A page that cannot be read is a reply saying what the fetch and the browser each returned, and which other vendor surfaces you tried. It is not a guess and not a new issue, the thread you are already in is the issue.
...
4. Call `github__find_related` with the slug before you push. This turn pushes to a branch it opens itself and cannot add a commit to one another run opened, so a tool that already has a pull request open still gets its own `agent/<slug>-outdated-<YYYY-MM-DD>` branch: link the open pull request in the body and say what this one changes on top of it, so the two get merged together or one gets dropped by a person.
...
A wrong or missing logo is a data report like any other, and `public/logos/<slug>.png` is the file. `contributing` says where the image comes from: the vendor's own favicon or GitHub organization avatar, never a logo lifted off a page that happens to show it. A wrong one is worth replacing. A wrong one you cannot replace with the vendor's own is worth saying so in the reply: no tool of yours deletes a file, and once a person removes it the card falls back to `icon` and then to the initial.
