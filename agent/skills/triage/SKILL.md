...
Bring the pull request refs in once, they are not part of a normal clone:
...
- A page that could not be read: read it again the way pricing-watch does, the fetch first and the browser second. If it reads now, handle the tool normally and close the issue with `github__close_issue`, saying what was read and linking the pull request when one came out of it. If it still refuses, leave the issue open and say so.
