...
4. Decide:
   - **Material change**: write the new snapshot, edit `content/tools/<slug>.yml` to match the page (tiers, prices, included, overage, limits, notes), bump `verified_at` on that source line to today, run `pnpm validate`, then push both paths with `github__push_files` on branch `agent/<slug>-pricing-<YYYY-MM-DD>` and message `data(<slug>): <what changed>`, and open a draft PR with the before and after values and the vendor URL. Call `github__find_related` with the slug first: if a pull request for this tool is open, push to the `branch` it returns instead of starting a new one.
