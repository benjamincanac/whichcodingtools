...
1. Fetch every pricing source, one capture each: `node /workspace/bin/page-text.mjs <url> > /tmp/<slug>.txt`. A tool can list more than one URL with `covers: [pricing]`, a self-serve page and an enterprise page, and the freshness badge takes the newest date across them, so a source skipped today goes stale with nothing on the site to show it. The script reads the origin's robots.txt first and exits with code 3 when a `Disallow` for `whichcodingtools-agent` or `*` covers the page. That tool is **reserved**: the vendor asked crawlers to stay away in the one machine-readable form there is, so do not open the page in the browser, do not look for another surface and do not open an issue. Note the rule from stderr for the report and move to the next tool. A 403 or a 429 is a different thing, a wall on one surface, and the fallback below still applies to it. Read the output. If the command fails (HTTP error, timeout) or the text has no prices at all (a client-rendered page), open the URL with `browser__navigate`, read it with `browser__read`, and pipe the rendered text back through the same script: `node /workspace/bin/page-text.mjs --stdin <url> > /tmp/<slug>.txt`. Never type a snapshot by hand. The header and the fence come from the script, `pnpm validate` rejects a file that does not match it, and a fence typed from memory is how page text ends up outside the guard that marks it as data. If the browser cannot read it either, the page is not the last word: look for the same figures on another page the same vendor publishes, its docs, billing docs, help center, changelog or developer pricing page. A 403 or a 429 is bot protection sitting on the marketing domain, and vendors rarely put the same wall on `docs.<domain>`. When one of those answers, read it, and change the pricing `sources[].url` to it in the PR with a line in the body saying the marketing page is blocked. Only when no surface the vendor owns answers is the tool **unreadable**. Never a third-party comparison, whatever the page says.
   When the page hides prices behind a toggle, a Pro / Pro+ / Ultra switch, monthly against yearly, a Standard / Premium seat, one capture only holds one state. Read each state in the browser and pipe each one to its own file: `pricing.txt` for the state the page opens on, `pricing-<state>.txt` for the others. Reach the toggle with `browser__find` on the label it shows rather than a ref from an earlier snapshot: the page rewrites itself every time a state changes, and a ref that outlives its snapshot fails the click and leaves you reading the state you already had. The figure check in step 4 reads all of them, so a price with no capture behind it cannot be pushed.
...
4. Decide:
   - **Material change**: write the new snapshots, edit `content/tools/<slug>.yml` to match the page (tiers, prices, included, overage, limits, notes), bump `verified_at` on that source line to today, run `pnpm validate`, hand the diff and the captures to `reviewer` as the `contributing` skill describes and fix what comes back, then push both paths with `github__push_files` on branch `agent/<slug>-pricing-<YYYY-MM-DD>` and message `data(<slug>): <what changed>`, and open a PR with the before and after values and the vendor URL. Call `github__find_related` with the slug first: if a pull request for this tool is open, push to the `branch` it returns instead of starting a new one, and rewrite its body with `github__update_pull_request` so the before and after still describe the branch.
...
- Never change `features`, `wraps`, `license` or `models` in this sweep. Pricing only. Put anything else you noticed in the PR body as a note for a human. A `description` counts as pricing when it states how the tool bills and the page you just read disagrees, and only that clause changes.
...
End with one paragraph: how many tools were checked, how many PRs opened or updated with links, how many issues opened with links, how many unreadable and which, how many reserved by robots.txt and which, with the rule each matched. No table, no restating the rules.
