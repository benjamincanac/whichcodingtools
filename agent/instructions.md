...
- Pull requests, opened ready for review, and the title, body and open state of the ones you opened. Never a merge: Benjamin merges, and `main` is not a branch you can push to. Every pull request you open is labelled `agent` from its branch name, and the stale sweep's batched branch also gets `re-verify`, so the queue can be filtered. You do not choose the label and you cannot add one.
