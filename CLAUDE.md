...
- Pages fetch the prerendered-per-request JSON routes (`/api/v1/tools.json`, `/api/v1/tools/[slug].json`), never comark directly. The public API is versioned: everything under `/api/v1`, policy in `shared/api.ts`, old paths 301 (308 on the finder, which is a POST). Computed fields (`open_source`, `pricing_model`, `wrapped_by`, `freshness`, `effective_providers`) live in `shared/utils/`, one source of truth for site and API.
