...
export default defineEventHandler(async (event) => {
  ...
  const paths = new Set<string>(['/', '/tools', '/compare', '/llms.txt', '/llms-full.txt', '/sitemap.xml', '/sitemap.md', '/api/v1/tools.json', '/api/compare.json', '/api/content/list', '/api/__sitemap__/urls'])
  ...
  for (const slug of touched) {
    ...
    paths.add(`/api/v1/tools/${slug}.json`)
